...
class EFLanguageList extends HTMLElement {
    ...
    render() {
        ...
        this.innerHTML = languages.length
            ? `<ul class="language-list">${languages.map((item) => `<li>${item.flag || "🌍"} ${item.name || item.code.toUpperCase()} — ${item.setupComplete && (item.journeyLabel || item.level) ? (item.journeyLabel || item.level) : "Diagnóstico pendente"}</li>`).join("")}</ul>`
